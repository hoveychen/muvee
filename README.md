...
Traefik is now listening on 443. Open `https://www.BASE_DOMAIN` in your browser and sign in with Google.

**3 — Register worker nodes**

Run the following on each worker machine (replace the URL and domain):
...
Traefik 开始监听 443 端口。在浏览器中打开 `https://www.BASE_DOMAIN`，使用 Google 账号登录。

**第 3 步 — 注册工作节点**

在每台工作机器上执行（替换 URL 和域名）：
