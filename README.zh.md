...
- **项目级访问控制** — 把项目切成 `private` 即可限定只允许白名单用户访问已部署的服务。Owner 可以看到 Recent Visitors（谁在用服务）和 Pending Requests（被拒用户从拒绝页一键提交的访问申请，approve 即时入白名单）。
- **Adhoc 隧道** — 使用 `muveectl tunnel 8080` 将本地端口直接发布到公网。根据工作目录和端口自动生成易读的子域名（如 `t-bold-fox.example.com`），相当于自托管的 ngrok。
