...
func main() {
	...
	// Background: process completed build tasks → dispatch deploy
...
func checkBuildCompletions(ctx context.Context, st *store.Store, sched *scheduler.Scheduler) {
	// Find deployments in "building" status that have a completed build task
	...
	for _, r := range rows2 {
		...
		// Mark build task as "processed" to avoid re-processing
