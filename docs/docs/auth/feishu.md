...
:::note Multi-domain
If you serve muvee under multiple base domains (`BASE_DOMAINS`), register this callback for **each** domain (e.g. also `https://app.muvee.ai/auth/feishu/callback`). See [Configuration → Multi-domain](../configuration#multi-domain).
:::
