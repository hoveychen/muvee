...
| `GOOGLE_REDIRECT_URL` | `http://localhost:8080/auth/google/callback` | OAuth2 回调 URL |
| `ALLOWED_DOMAINS` | _（允许所有）_ | 允许登录的邮箱域名，逗号分隔（如 `company.com`） |
| `ADMIN_EMAILS` | — | 逗号分隔的 Google 账号列表，登录时自动提升为 `admin` 角色，可访问 `traefik.BASE_DOMAIN` |
